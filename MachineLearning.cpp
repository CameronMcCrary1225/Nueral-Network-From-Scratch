...
//p is expected, q is actual probability distribution
...
bool mat_softmax_add_grad(matrix* out, const matrix* softmax_out, const matrix* grad) {
...
bool mat_cross_entropy_add_grad(matrix* p_grad, matrix* q_grad, const matrix* p, const matrix* q, const matrix* grad) {
...
void draw_mnist_digit(const f32* data) {
	for (u32 y = 0; y < 28; y++) {
		for (u32 x = 0; x < 28; x++) {
			...
			printf("\x1b[48;5;%dm  ", col); // ANSI escape for background color
		}
		printf("\n");
	}
...
model_var* mv_create(mem_arena* arena, model_context* model, u32 rows, u32 cols, u32 flags) {
	model_var* out = PUSH_STRUCT_NZ(arena, model_var);
	memset(out, 0, sizeof(model_var));
...
model_var* mv_relu(mem_arena* arena, model_context* model, model_var* input, u32 flags) {
	return _mv_unary_impl(arena, model, input, input->val->rows, input->val->cols, flags, MV_OP_RELU);
}
...
model_var* mv_cross_entropy(mem_arena* arena, model_context* model, model_var* p, model_var* q, u32 flags) {
	...
	return _mv_binary_impl(arena, model, p, q, p->val->rows, p->val->cols, flags, MV_OP_CROSS_ENTROPY);
}
...
model_program model_prog_create(mem_arena* arena, model_context* model, model_var* out_var) {
	...
	model_program prog{  PUSH_ARRAY_NZ(arena, model_var*, out_size), out_size };
...
void model_prog_compute_grads(model_program* prog) {
	...
	for (i64 i = (i64)prog->size - 1; i >= 0; i--) {
		...
		if (num_inputs == 2 && (a->flags & MV_FLAG_REQUIRES_GRAD) != MV_FLAG_REQUIRES_GRAD && (b->flags & MV_FLAG_REQUIRES_GRAD) != MV_FLAG_REQUIRES_GRAD){
			continue;
		}
		switch (cur->op) {
			case MV_OP_NULL:
...
void create_mnist_model(mem_arena* arena, model_context* model) {
	// Input layer: 784 x 1
	model_var* input = mv_create(arena, model, 784, 1, MV_FLAG_INPUT | MV_FLAG_REQUIRES_GRAD);

	// Layer 0 weights/biases: 16 hidden neurons
	...
	mat_fill_rand(b0->val, -bound0, bound0);

	// Layer 1 weights/biases: 16 hidden neurons
	...
	mat_fill_rand(b1->val, -bound1, bound1);

	// Layer 2 weights/biases: output layer 10 classes
	...
	mat_fill_rand(b2->val, -bound2, bound2);
